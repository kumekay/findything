/* Modified for Intel IOT Roadshow at 27th of June 2015 
    SPB, Demola coworking place */
...
#define ADV_COMPANY_IDENTIFIER     0x00AB                // Идентификатор компании
#define ADV_DEVTYPE                0xFE                  // Идентификатор типа устройства для ноды
...
/* Bacons advertising data packet */
typedef __packed struct                     /**< Information advertised by the Beacon. */
{
    uint16_t    company_id;         /** Should be 0x004C for Apple Inc */
    uint8_t     dev_type;           /**< Manufacturer specific information. 0x02 refers to Beacon. */
    uint8_t     data_length;        /** Should be 0x15 for Beacons */    
    uint8_t     uuid[16];           // 128 bit UUID value. 
    uint16_t    major_val;          /**< Major value used to identify Beacons. */ 
    uint16_t    minor_val;          /**< Minor value used to identify Beacons. */ 
    uint8_t     meas_rssi;          /**< The Beacon's measured RSSI at 1 meter distance in dBm. */
} beacon_advdata_t;

/* Nodes advertising struct type */
typedef __packed struct 
{
    uint8_t     dev_type;              /**< Manufacturer specific information. 0xFE refers to Beacon. */
    uint8_t     data_length;           /** Data lenght for least structure */    
    uint16_t    beacon_major_val;      /**< Major value used to identify Beacons. */ 
    uint16_t    beacon_minor_val;      /**< Minor value used to identify Beacons. */ 
    uint8_t     beacon_meas_rssi;      /**< The Beacon's measured RSSI */
} node_advdata_t;       
...
static node_advdata_t               node_advdata;     // Данные от ноды для передачи через Advertisment packets
...
{
    // Modified for Intel IOT Roadshow
                
    uint32_t                err_code;
    uint32_t                n;
    const ble_gap_evt_t   * p_gap_evt = &p_ble_evt->evt.gap_evt;
    const beacon_advdata_t * p_beacon_adv = NULL;
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_ADV_REPORT:
        {
            ...
            {
                if (type_data.data_len == 25 && 
                    sizeof(beacon_advdata_t) == 25) 
                {
                    p_beacon_adv = (const beacon_advdata_t*)type_data.p_data;
                    
                    // Проверка соответствия формату iBeacon
                    if (p_beacon_adv->company_id == 0x004C && 
                        p_beacon_adv->dev_type   == 0x02 && 
                        p_beacon_adv->data_length == 0x15 &&
                        // Отбираем нужный нам Beacon
                        p_beacon_adv->major_val == 0x0400 && p_beacon_adv->minor_val == 0x0100)
                    {
                        printf("beacon ");
                        /* for (n = 0; n < 16; n++) {
                            printf("%02X ", p_beacon_adv->uuid[n]);
                        }   */
                        printf(" rssi %d, maj %04X, min %04X\r\n", 
                                p_gap_evt->params.adv_report.rssi,
                                p_beacon_adv->major_val,
                                p_beacon_adv->minor_val);
                        
                        // Полученные данные о iBeacon- в пакет advertising
                        node_advdata.beacon_major_val = p_beacon_adv->major_val;
                        node_advdata.beacon_minor_val = p_beacon_adv->minor_val;
                        node_advdata.beacon_meas_rssi = p_gap_evt->params.adv_report.rssi;
                    ...
                    if (p_beacon_adv->company_id == 0x004C && 
                        p_beacon_adv->dev_type   == 0x02 && 
                        p_beacon_adv->data_length == 0x15 &&
                        p_beacon_adv->major_val == 0x4600)
                    {
                        if (p_beacon_adv->minor_val & 0x0100) {
...
static void advertising_init(void)
{
    ...
    // Build and set node manufacturer specific advertising data.
    memset(&node_advdata, 0, sizeof(node_advdata));
    
    node_advdata.dev_type           = ADV_DEVTYPE;
    node_advdata.data_length        = sizeof(node_advdata) - sizeof(node_advdata.dev_type) - sizeof(node_advdata.dev_type);
    node_advdata.beacon_major_val   = 0xFF;
    node_advdata.beacon_minor_val   = 0xFF;
    node_advdata.beacon_meas_rssi   = 0x80;
        
    manuf_specific_data.company_identifier = ADV_COMPANY_IDENTIFIER;
    manuf_specific_data.data.p_data = (uint8_t *) &node_advdata;
    manuf_specific_data.data.size   = sizeof(node_advdata);
...
static void advertising_update(void)
{
    ...
    // Update node manufacturer specific advertising data.
    node_advdata.dev_type           = ADV_DEVTYPE;
    node_advdata.data_length        = sizeof(node_advdata) - sizeof(node_advdata.dev_type) - sizeof(node_advdata.dev_type);
...
int main(void)
{
    ...
    uart_init();
    printf("Node example for Intel IOT Roadshow\r\n");
    ble_stack_init();
    device_manager_init(erase_bonds);
    db_discovery_init();
    advertising_init();

    // Modified for iBeacon UUID for Intel RoadShow
    scan_start();
    advertising_start();
